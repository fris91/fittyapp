...
| --- | --- | ---: | --- | --- |
| `fitty-cp-01` | Intel Core i7, 6th generation, amd64 | 16 GB | Control-plane | `192.168.1.192` |
| `fitty-worker-01` | Intel Core i5, 4th generation, amd64 | 16 GB | Worker | `192.168.1.41` |
...
| Worker | `fitty-worker-01` / `192.168.1.41` |
...
```bash
sudo kubeadm join 192.168.1.191:6443 \
  --token <token> \
...
sudo sed -i "/plugins.'io.containerd.cri.v1.images'.registry/,+2 s#config_path = .*#config_path = '/etc/containerd/certs.d'#" /etc/containerd/config.toml
sudo systemctl restart containerd
sudo systemctl status containerd --no-pager
```
...
## Build and Deploy from a Windows Development Machine

If the Fitty repository is on your Windows machine, use Windows only as the build/deploy workstation. The Kubernetes nodes still run Linux natively with containerd.

You need one of these on Windows:

- Docker Desktop with Linux containers, only on the Windows development machine.
- Or WSL2 Ubuntu with Docker Engine installed inside WSL2.
- Or Podman Desktop configured to build Docker-compatible images.

The simplest path is Docker Desktop on Windows for builds, while the two laptops stay Docker-Desktop-free.

### 1. Make Cluster Names Resolve from Windows

Edit Notepad as Administrator and open:

```text
C:\Windows\System32\drivers\etc\hosts
```

Add your real cluster IPs:

```text
192.168.1.192 fitty-cp-01
192.168.1.41 fitty-worker-01
192.168.1.192 fitty.local api.fitty.local keycloak.fitty.local kafka-ui.fitty.local
```
...
```powershell
ping fitty-cp-01
curl.exe http://fitty-cp-01:5000/v2/_catalog
```

Expected registry response before pushing images:

```json
{"repositories":[]}
```

### 2. Allow Docker on Windows to Push to the HTTP Registry

Docker blocks plain HTTP registries unless they are marked as insecure.
...
If the file already contains other keys, keep them and only add the `insecure-registries` field.
...
### 3. Copy kubeconfig from the Control Plane to Windows

On `fitty-cp-01`, print the kubeconfig:
...
In `$HOME\.kube\config`, replace the API server address if needed:

```yaml
server: https://192.168.1.192:6443
```
...
If `kubectl` is missing on Windows, install it:
...
docker build -t fitty-cp-01:5000/fitty/meal-service:local .\services\meal-service
docker build -t fitty-cp-01:5000/fitty/notification-service:local .\services\notification-service
docker build -t fitty-cp-01:5000/fitty/web-app:local .\frontend-web
```

If you use WSL2, run the Linux-style Makefile targets from inside WSL2:

```bash
cd /mnt/e/workspace/fittyApp
make k8s-build-images REGISTRY=fitty-cp-01:5000 IMAGE_TAG=local
```

### 5. Push Images to the Control-Plane Registry
...
docker push fitty-cp-01:5000/fitty/meal-service:local
docker push fitty-cp-01:5000/fitty/notification-service:local
...
```powershell
curl.exe http://fitty-cp-01:5000/v2/_catalog
curl.exe http://fitty-cp-01:5000/v2/fitty/api-gateway/tags/list
```

### 6. Deploy to Kubernetes from Windows
...
kubectl get svc -A
curl.exe http://fitty-cp-01:30080/actuator/health
curl.exe http://fitty-cp-01:30000
```

### 7. If Images Do Not Pull

On both Linux nodes, verify containerd registry trust:

```bash
sudo cat /etc/containerd/certs.d/fitty-cp-01:5000/hosts.toml
sudo systemctl restart containerd
...
```powershell
kubectl delete pod -n fitty-app -l app=api-gateway
...
Fitty uses Keycloak as the local Identity Provider. The manifests are under `infra/k8s/local/keycloak/` and deploy:

- `keycloak-postgres`: dedicated PostgreSQL persistence for Keycloak.
- `keycloak`: Keycloak server exposed through NodePort `30081`.
- `fitty` realm import with local clients and roles.

Apply Keycloak by itself:
...
```bash
kubectl get svc -n fitty-system keycloak
curl http://fitty-cp-01:30081/realms/fitty/.well-known/openid-configuration
```

Default local admin console:

```text
URL:      http://fitty-cp-01:30081
User:     admin
Password: admin123
Realm:    fitty
```

Default local test users imported with the realm:

| Username | Password | Role | Subscription |
| --- | --- | --- | --- |
| `fitty.user` | `user123` | `FITTY_USER` | `FREE` |
| `fitty.admin` | `admin123` | `FITTY_ADMIN` | `GOLD` |

Local OIDC clients:

| Client | Type | Use |
| --- | --- | --- |
| `fitty-web` | public, Authorization Code + PKCE | Web app |
...
```bash
kubectl get pods -n fitty-system -o wide
...
kubectl get svc -n fitty-system
curl http://fitty-cp-01:30081/realms/fitty/.well-known/openid-configuration
...
| Keycloak | `300m`, `768Mi` | `1`, `1536Mi` |
| Keycloak PostgreSQL | `150m`, `256Mi` | `500m`, `512Mi` |
| PostgreSQL | `250m`, `512Mi` | `1`, `1Gi` |
| MongoDB | `250m`, `512Mi` | `1`, `1Gi` |
| Kafka UI | `100m`, `256Mi` | `500m`, `512Mi` |
| Web app | `100m`, `128Mi` | `300m`, `256Mi` |

The local manifest uses `bitnamilegacy/kafka:3.7.0` because older Bitnami Kafka tags may no longer be available under `bitnami/kafka`.
...
make k8s-port-forward-keycloak
make k8s-status
make k8s-logs
make k8s-down
```
